...
class TeacherController {
  ...
  static async createTeacher(req, res) {
    try {
      ...
      if (!hoTen || !CCCD || !diaChi || !ngayBatDau || !chuyenMon || !chucVu || !SDT || !email) {
      ...
      // Bước 1: Tạo tài khoản mặc định
      const accountResult = await UserModel.createDefaultAccount({
        hoTen,
        loaiTaiKhoan,
        email,
        phone: SDT,
      ...
      if (!accountResult.success) {
        return res.status(400).json({
          success: false,
          message: "Không thể tạo tài khoản: " + accountResult.message,
        });
      }
      ...
      if (result.affectedRows > 0) {
        res.status(201).json({
          ...
          data: {
            ...
            account: {
              maTaiKhoan: accountResult.data.maTaiKhoan,
              tenDangNhap: accountResult.data.tenDangNhap,
              defaultPassword: accountResult.data.defaultPassword,
              loaiTaiKhoan: accountResult.data.loaiTaiKhoan
            },
            loginInfo: {
              message: `Tài khoản đã được tạo cho giáo viên ${hoTen}`,
              username: accountResult.data.tenDangNhap,
              password: accountResult.data.defaultPassword,
  ...
  static async updateTeacher(req, res) {
    try {
      ...
      if (userRole !== 'giaovu' && userRole !== 'bangiamhieu') {
        return res.status(403).json({
          success: false,
          message: "Chỉ giáo vụ và ban giám hiệu mới có quyền cập nhật thông tin giáo viên",
        });
      }
      ...
      if (!hoTen || !CCCD || !diaChi || !chuyenMon || !chucVu || !SDT || !email) {
        return res.status(400).json({
          success: false,
  ...
  static async deleteTeacher(req, res) {
    try {
      ...
      if (userRole !== 'giaovu' && userRole !== 'bangiamhieu') {
        return res.status(403).json({
          success: false,
          message: "Chỉ giáo vụ và ban giám hiệu mới có quyền nghỉ việc giáo viên",
        });
      }
      ...
      if (result.affectedRows > 0) {
        ...
        const [teacher] = await pool.execute("SELECT maTaiKhoan FROM giaovien WHERE maGV = ?", [maGV]);
        if (teacher.length > 0) {
  ...
  static async getTeachers(req, res) {
    try {
      const { page = 1, limit = 10, chuyenMon, chucVu, search, showInactive = false } = req.query;
      const { loaiTaiKhoan: userRole } = req.user;
