...
class ClassController {
  ...
  static async createClass(req, res) {
    ...
    if (!khoi || !maGVChuNhiem || !siSo) {
      return res.status(400).json({
        success: false,
        message: "Thiếu thông tin bắt buộc (khối, gvcn, sĩ số).",
      });
    }
  ...
  static async updateClass(req, res) {
    ...
    if (!maGVChuNhiem || !siSo) {
      return res
        .status(400)
        .json({ success: false, message: "Thiếu thông tin (gvcn, sĩ số)." });
    }

    try {
      const success = await ClassModel.update(id, {
        maGVChuNhiem,
        siSo: parseInt(siSo),
      });
      if (!success) {
        return res
          .status(404)
          .json({ success: false, message: "Không tìm thấy lớp để cập nhật." });
      }
      res
        .status(200)
        .json({ success: true, message: "Cập nhật lớp thành công." });
    } catch (error) {
  ...
  static async deleteClass(req, res) {
    ...
    try {
      ...
      if (!success) {
        return res
          .status(404)
          .json({ success: false, message: "Không tìm thấy lớp để xóa." });
      }
